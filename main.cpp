...
//std::string classes[81] = { // Microsoft COCO dataset segmentation 81 classes label
...
#define CLASSNUMBER 92
std::string classes[92] = { // Microsoft COCO dataset segmentation 92 classes label
...
int main(int argc, char *argv[]) {
	try {
		...
		const float PROBABILITY_THRESHOLD = 0.4f;
		...
		for (size_t box = 0; box < BOXES; ++box) {
			...
			if (prob > PROBABILITY_THRESHOLD) {
				...
				if (((int)class_id) < CLASSNUMBER) {
					sprintf(label, "%s:%.2f", classes[class_id], prob);
				}
				else {
					sprintf(label, "unknown:%.2f", prob);
				}

				//Display the label at the top of the bounding box
				int baseLine;
				cv::Size labelSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 1.0, 1, &baseLine);
				roi.y = std::max(roi.y, labelSize.height);
				//cv::rectangle(output_images[batch], cv::Point(roi.x, roi.y - round(1.5*labelSize.height)),
				//	cv::Point(roi.x + round(1.5*labelSize.width), roi.y + baseLine), cv::Scalar(0, 0, 255), cv::FILLED);
				cv::putText(output_images[batch], label, cv::Point(roi.x, roi.y), 
		...
		double time1 = (t1 - t0) / cv::getTickFrequency();
		slog::info << "Command parse and load inference engine time = " << time1 << " sec." << slog::endl;
		double time2 = (t2 - t1) / cv::getTickFrequency();
		slog::info << "Load network  (bin/xml) time = " << time2 << " sec." << slog::endl;
		double time3 = (t3 - t2) / cv::getTickFrequency();
		slog::info << "Prepare input blob time = " << time3 << " sec." << slog::endl;
		double time4 = (t4 - t3) / cv::getTickFrequency();
		slog::info << "Prepare output blob time = " << time4 << " sec." << slog::endl;
		double time5 = (t5 - t4) / cv::getTickFrequency();
		slog::info << "Load model to device time = " << time5 << " sec." << slog::endl;
		double time6 = (t6 - t5) / cv::getTickFrequency();
		slog::info << "Create inference request time = " << time6 << " sec." << slog::endl;
		double time7 = (t7 - t6) / cv::getTickFrequency();
		slog::info << "Set input data time = " << time7 << " sec." << slog::endl;
		double time8 = (t8 - t7) / cv::getTickFrequency();
		slog::info << "Do inference time = " << time8 << " sec." << slog::endl;
		double time9 = (t9 - t8) / cv::getTickFrequency();
		slog::info << "Postprocess output blob time = " << time9 << " sec." << slog::endl;
		double total = (t9 - t0) / cv::getTickFrequency();
		slog::info << "Total time = " << total << " sec." << slog::endl;
	}
